...
Settings are stored in ~/.config/continuous/config.yaml.
Secret credentials are stored in the OS keychain when available.

You can also set values via environment variables with the CONTINUOUS_ prefix
(e.g., CONTINUOUS_API_KEY) or pass them as flags to individual commands.
	...
	if output.IsAgentMode() {
		return output.AgentModeError(cmd,
			...
			[]string{
				...
				fmt.Sprintf("Run '%s whoami' to verify current authentication", "continuous"),
			},
		)
	}
