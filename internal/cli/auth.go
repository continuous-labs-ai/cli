...
func initAuthCmd(parent *cobra.Command) error {
	authCmd := &cobra.Command{
		...
		Long: `Manage authentication credentials for continuous.
...
Subcommands:
  ...
  logout  - Clear all stored credentials`,
	...
	authCmd.AddCommand(&cobra.Command{
		...
		Long: `Interactively configure authentication credentials for continuous.
...
func runAuthLoginCmd(cmd *cobra.Command, args []string) error {
	...
	if output.IsAgentMode() {
		return output.AgentModeError(cmd,
			...
			[]string{
				fmt.Sprintf("Set credentials via environment variables (prefix: %s_)", "CONTINUOUS"),
				"Pass credentials directly as CLI flags for each command",
				fmt.Sprintf("Run '%s auth whoami' to verify current authentication", "continuous"),
			},
		)
	}
	...
	if noInteractive, _ := cmd.Flags().GetBool("no-interactive"); noInteractive {
		...
		var authApiKeyAuth string

		accessible := !authIsInteractive(cmd)

		fields := []huh.Field{
			huh.NewInput().
				Title("Use a Continuous API key. Send it in the Authorization header as a Bearer token.").
...
func runAuthLogoutCmd(cmd *cobra.Command, args []string) error {
	...
	if config.KeyringAvailable() {
		_ = config.DeleteKeyringValue("api-key-auth")
	}
	cfg.Security.ApiKeyAuth = ""
