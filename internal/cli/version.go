...
// which propagates the value here (see cmd/continuous/main.go):
//
//	go build -ldflags "-X main.version=x.y.z" ./cmd/continuous
var Version = "0.0.1"
...
func initVersionCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		...
		Long: `Print the current version of the continuous CLI.

The version defaults to the SDK version set during generation, but can be
overridden at build time using Go linker flags:

  go build -ldflags "-X main.version=x.y.z -X main.buildTime=$(date -u +%Y-%m-%dT%H:%M:%SZ)" ./cmd/continuous`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if usage.UsageRequested(cmd) {
				return usage.EmitSchema(cmd, cmd.OutOrStdout())
			}
			if _, err := fmt.Fprintf(cmd.OutOrStdout(), "continuous %s\n", Version); err != nil {
				return err
			}
			if BuildTime != "" {
